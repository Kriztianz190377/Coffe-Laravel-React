...
interface ProductSumamaryProps{
    product:ProductProps
}


export const SummaryProduct = ({product}:ProductSumamaryProps) => {
  
const{handleEditionQuantity,handleDeletProductOrder}=useKiosk()
  ...
  const {id,name, price, quantity=1}=product
    ...
    <div className="shadow space-y-1 p-4 bg-white">
      ...
      <div className="flex justify-between gap-2 pb-4">
        <button
        onClick={()=>handleEditionQuantity(id)}
          type="button"
        ...
        <button
        onClick={()=>handleDeletProductOrder(id)}
          type="button"
